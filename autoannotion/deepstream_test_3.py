...
def tiler_sink_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            ...
            # n_frames = draw_bounding_boxes(n_frame, obj_meta, obj_meta.confidence)
            ...
            if saved_count["stream_{}".format(frame_meta.pad_index)] % 30 == 0 and (
                     obj_meta.confidence > MAX_CONFIDENCE):
                if is_first_obj:
                    is_first_obj = True
                    ...
                    frame_copy = np.array(n_frame[0], copy=True, order='C')
        ...
        if save_image:
            if frame_meta.frame_num % 1 == 0:
                
                img_path = "{}/stream_{}/frame_{}.jpg".format(folder_name, frame_meta.pad_index, frame_number)
                cv2.imwrite(img_path, frame_copy)
            generated(f"stream_{frame_meta.pad_index}",frame_number,os.getcwd(),n_frame[1],folder_name)
...
def draw_bounding_boxes(image, obj_meta, confidence):
    ...
    lst.append([top,left,width,height,obj_name])
    print(lst)
